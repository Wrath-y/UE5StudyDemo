...
void AShotgun::FireShotgun(const TArray<FVector_NetQuantize>& HitTargets)
{
	...
	if (MuzzleFlashSocket)
	{
		...
		TMap<ACorpsePartyCharacter*, uint32> HeadShotHitMap;
		for (FVector_NetQuantize HitTarget : HitTargets)
		{
			...
			if (CorpsePartyCharacter)
			{
				...
				if (bHeadShot)
				{
					if (HeadShotHitMap.Contains(CorpsePartyCharacter)) HeadShotHitMap[CorpsePartyCharacter]++;
					else HeadShotHitMap.Emplace(CorpsePartyCharacter, 1);
				}
				else
				{
					if (HitMap.Contains(CorpsePartyCharacter)) HitMap[CorpsePartyCharacter]++;
					else HitMap.Emplace(CorpsePartyCharacter, 1);
				}
				if (ImpactParticles)
				{
		...
		TMap<ACorpsePartyCharacter*, float> DamageMap;
